...
file_path = 'env/street_map.yaml'
with open(file_path, 'r') as file:
    data = yaml.load(file, Loader=yaml.FullLoader)

json_path = 'street.json'
...
num_s = [64, 128, 256, 512]

for num_agent in num_s:
    if num_agent not in res.keys():
            res[num_agent] = {}
            mean[num_agent] = {}
            standard_deviation[num_agent] = {}
    for name, _ in zip(data.keys(), data.values()):
        res[num_agent][name] = []
        print(name)
        res[num_agent][name].extend(test_on_map(algo, int(num_agent), name))
