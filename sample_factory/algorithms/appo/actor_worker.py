...
class ActorState:
    ...
    def __init__(
        ...
        self.policy_output_indices = np.cumsum(self.policy_output_sizes)  # save policy_output_indices
        self.policy_output_tensors = policy_output_tensors   # save policy_output_tensor
    ...
    def record_env_step(self, reward, done, info, rollout_step):
        ...
        report = None
        if done:
            last_episode_true_reward = info.get('true_reward', self.real_episode_reward)
    ...
    def episodic_stats(self, last_episode_true_reward, last_episode_extra_stats):
        ...
        self.last_episode_reward = self.last_episode_duration = self.real_episode_reward = 0
        return report
...
class VectorEnvRunner:
    ...
    def init(self):
        ...
        for env_i in range(self.num_envs):
            ...
            actor_states_env, episode_rewards_env = [], []   # agents info
    ...
    def _process_env_step(self, new_obs, rewards, dones, infos, env_i):
        ...
        for agent_i in range(self.num_agents):
            ...
            )   # load step info to shared_buffer
