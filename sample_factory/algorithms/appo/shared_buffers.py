...
class SharedBuffers:
    def __init__(self, cfg, num_agents, obs_space, action_space):
        ...
        obs_space.spaces['attention_mask']   = np.zeros(64)
        obs_space.spaces['ids_oth']          = np.zeros(64)
        ...
        self._tensors['rewards'].fill_(-42.42)  
        ...
        self._tensors['policy_id'].fill_(-1)  
        ...
        for po in policy_outputs:
            self._tensors[po.name] = self.init_tensor(torch.float16, [po.size])   
    ...
    def get_trajectory_buffers(self, num_buffers: int, timing = None):
        """
